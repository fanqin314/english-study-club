...
(function (global) {
  ...
  // 全文翻译
  // 输出契约改为「带序号逐行格式」：[序号] 译文，每行一句，按序号与原文句子一一对应。
  // 相比 JSON 数组，模型更易稳定输出带序号列表；且可按序号归位，规避数组元素个数不匹配导致的错位。
  function buildFullTranslationMessages(text) {
    return [
      {
        role: 'system',
        content:
          '你是逐句翻译引擎。将用户提供的英文文章逐句翻译成中文。严格按以下格式输出，每行只输出一句的中文翻译，行首用英文方括号标出序号：\n' +
          '[1] 第一句的中文翻译\n' +
          '[2] 第二句的中文翻译\n' +
          '...\n' +
          '规则：\n' +
          '1. 序号从 1 开始连续递增，与原文句子一一对应（按英文句号 .、问号 ?、感叹号 ! 统计句子总数）。\n' +
          '2. 分号 ;、冒号 :、逗号 , 都不是句子边界；一个句子即使包含多个分句，也必须合并成一条完整的译文，禁止拆成多行。\n' +
          '3. 译文内不要再出现序号，不要用引号把整句译文包裹起来。\n' +
          '4. 严禁把多句译文写进同一行，也严禁在一行里用逗号或分号连接多句译文。\n' +
          '只输出这些行，不要任何解释、注释或 Markdown 代码块。'
      },
