    ...
    return [
      {
        ...
        content:
          '将以下英文文章逐句翻译成中文。规则：\n' +
          '1. 只有英文句号(.)、问号(?)、感叹号(!)是句子结束标志；分号(;)、冒号(:)、逗号(,)都不是句子边界，严禁在其后插入 [SENTENCE_END]。\n' +
          '2. 一个原始句子即使包含分号/冒号连接的多个分句，也必须合并输出为一条完整中文翻译，不能拆开。\n' +
          '3. 每句翻译之后紧跟一个 [SENTENCE_END] 分隔符。翻译条数必须与原文句子数（按句号/问号/感叹号统计）完全一致，不得多拆、不得漏句。\n' +
          '4. 文章开头的独立标题（如 "Of Studies"）不要单独输出，并入第一句翻译。\n' +
          '只返回翻译结果，不要任何解释。'
      },
